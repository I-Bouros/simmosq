...
def run_unit_tests():
    ...
    """
    tests = os.path.join('simmosq', 'tests')
...
def run_copyright_checks():
    ...
    # Recursively walk the simmosq directory and check copyright header
...
#"""
    ...
    for dirname, subdir_list, file_list in os.walk('simmosq'):
...
def doctest_rst_and_public_interface():
    ...
    # Import all public simmosq modules. We list
    # everything here for completeness. If a new module is added to
    # simmosq it should be imported here for this doctest.
    import simmosq
    ...
    # has been added to simmosq and should be imported above and included
    # in this list.
    simmosq_submodules = [
        'simmosq.version_info',
        ]

    doc_symbols = get_all_documented_symbols()

    check_exposed_symbols(simmosq, simmosq_submodules, doc_symbols)
...
def check_exposed_symbols(module, submodule_names, doc_symbols):
    ...
    if len(unexpected_modules) > 0:
        ...
        print('For python modules such as numpy you may need to confine the '
              ...
              'simmosq submodule, you will need to make %s (doctest) '
              'aware of this.'
              % __file__)
        print('FAILED')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run unit test for simmosq',
        epilog='To run individual unit tests, use e.g.'
               ' $ python3 simmosq/tests/test_dummy.py',
    )
    # Unit tests
    parser.add_argument(
        '--unit',
